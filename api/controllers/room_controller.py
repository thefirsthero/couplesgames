...
@router.post("/rooms/{room_id}/leave", response_model=RoomResponse)
...
@router.post("/rooms/{room_id}/questions", response_model=WouldYouRatherQuestionResponse)
async def add_question_to_room(room_id: str, question: WouldYouRatherQuestion, current_user: User = Depends(get_current_user)):
    room_ref = db.collection('rooms').document(room_id)
    room = room_ref.get()
    if not room.exists:
        raise HTTPException(status_code=404, detail="Room not found")
...
@router.get("/rooms/{room_id}/questions", response_model=List[WouldYouRatherQuestionResponse])
